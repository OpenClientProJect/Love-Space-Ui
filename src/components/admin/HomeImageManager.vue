            ...
            <el-form :model="carouselForm" label-width="100px" ref="carouselFormRef">
              ...
              <el-form-item label="视频" prop="video">
                <el-upload
                    ...
                    v-if="!carouselForm.video"
                >
                  <el-icon class="upload-icon">
                    <VideoCamera/>
                  </el-icon>
                  <div class="upload-text">点击上传视频
                  </div>
                </el-upload>
                <div v-else class="video-preview">
                  <video :src="carouselForm.video" controls class="preview-video"></video>
                  <div class="preview-actions">
                    <el-button type="danger" size="small" @click="carouselForm.video = ''">
                      删除
                    </el-button>
                  </div>
                </div>
...
import {Edit, Plus, Upload, Picture, VideoCamera} from '@element-plus/icons-vue'
import {useTokenStore} from '@/stores/token'
import {
  getBackgroundImageService,
  getHomeImageService,
  uploadHomeImageService,
  editHomeImageService,
  deleteHomeImageService,
  updateHomeImageService,
  addHomeImageService
} from '@/api/admin/adminhomeimage'
...
const uploadAction = '/api/file/uploadImage'
const uploadUrl = '/api/file/uploadVideo'
// 轮播图列表
...
const beforeVideoUpload = (file) => {
  ...
  if (!isMP4 && !isQuickTime) {
    ElMessage.error('上传视频只能是 MP4/MOV 格式!')
    return false
  }
  return true
}
...
const handleVideoSuccess = (response) => {
  if (response.code === 200 && response.data) {
    carouselForm.video = response.data
